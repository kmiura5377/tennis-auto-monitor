...
- GitHub Actions が **30分に1回**、Playwright（ヘッドレスブラウザ）で予約サイトを実際に操作し、直近5週間分の空き状況を取得します
...
予約サイトは利用規約で「過剰な自動アクセス」を明示的に禁止しており、検知した場合はアクセス遮断やアカウント停止を行うとしています。これを踏まえ、チェック頻度は15分ごとではなく **30分に1回** に抑えています。
...
├── .github/workflows/
│   └── monitor.yml             # 30分ごとにスクレイパーを実行するGitHub Actions
├── package.json
└── README.md
```
